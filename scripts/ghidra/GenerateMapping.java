...
public class GenerateMapping extends GhidraScript
{
    ...
    @Override public void run() throws Exception
    {
        ...
        {
            outputMapping = new File("/tmp/th07_mapping.csv");
        }
        if (outputMapping.getParentFile() != null)
        {
            outputMapping.getParentFile().mkdirs();
        }
        Files.write(outputMapping.toPath(), mappingData.getBytes());
        int lines = mappingData.split("\n").length;
        println("GenerateMapping: wrote " + lines + " functions to " + outputMapping);
    }
}
