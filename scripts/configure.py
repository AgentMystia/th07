...
def configure(build_type):
    with (SCRIPTS_DIR.parent / "build.ninja").open("w") as f:
        ...
        cl_common_flags = "/MT /EHsc /Gs /GS /DNDEBUG /Zi /I $builddir/autogenerated /I src /I 3rdparty/munit /I 3rdparty/Detours/src"
        ...
        writer.rule(
            "gendetours",
            "python scripts/generate_detours.py --input-def $builddir/th07.def --output $out",
        )
        writer.rule(
            "gendef",
        ...
        for rule in pbg4_sources:
            writer.build(
                "$builddir/" + rule + ".obj",
                "cc",
                "src/pbg4/" + rule + ".cpp",
                implicit=["$builddir/autogenerated/i18n.hpp"],
                variables={"cl_flags": "$cl_flags_pbg4"},
            )
            writer.build(
                "$builddir/objdiff/reimpl/" + rule + ".obj",
                "rename_symbols",
        ...
        writer.build(
            ...
            inputs=[
                "$builddir/" + x + ".obj"
                for x in (cxx_sources + pbg4_sources + ["stubs"])
            ],
            implicit=["scripts/gendef.py"],
        )
        objfiles = (
            ["$builddir/" + src + ".obj" for src in cxx_sources]
            + ["$builddir/" + src + ".obj" for src in pbg4_sources]
            + ["$builddir/th07.res", "$builddir/stubs.obj"]
        )
        if build_type in [BuildType.DIFFBUILD, BuildType.DLLBUILD]:
            objfiles += ["$builddir/globals.obj"]

        th07_link_libs = "dxguid.lib d3dx8.lib d3d8.lib dsound.lib winmm.lib kernel32.lib user32.lib dinput8.lib gdi32.lib winspool.lib comdlg32.lib advapi32.lib shell32.lib ole32.lib oleaut32.lib uuid.lib odbc32.lib odbccp32.lib"
        writer.build(
            "$builddir/th07e.exe",
            "link",
            inputs=objfiles,
            ...
            variables={
                "link_libs": th07_link_libs,
                "link_flags": "$th07_link_flags /debug /pdb:$builddir/th07e.pdb",
            },
        )
        ...
        writer.build(
            "$builddir/th07e-tests.exe",
            "link",
            inputs=test_objfiles + ["$builddir/munit.lib"],
            variables={
                "link_libs": th07_link_libs + " $builddir/munit.lib",
                "link_flags": "/debug /pdb:$builddir/th07e.pdb",
            },
        )
        ...
        writer.build(
            ...
            implicit=["$builddir/th07.def"],
            variables={
                "link_libs": th07_link_libs,
                "link_flags": "/DLL /debug /pdb:$builddir/th07e.pdb /export:DetourFinishHelperProcess,@1,NONAME /def:$builddir/th07.def /export:Direct3DCreate8 /export:malloc /export:calloc /export:realloc /export:??2@YAPAXI@Z /export:free /export:_msize",
            },
        )
