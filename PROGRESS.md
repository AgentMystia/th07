...
### EffectManager（第 17 模块，13 函数 objdiff，平均 **62.2%**，3×99%+）
orig 10.2KB / 13 函数（AddedCallback@0x41cdd4 ghidra overlap 未导出，4 函数待补）。
| 函数 | match% |
|---|---|
| Reset | **99.92%** |
...
| EffectCallbackAttractInit | 31.17%（地址修正后 0%→31%）|
| OnDraw | 36.46% |
| EffectManager (ctor) | 35.40% |
| EffectUpdateCallback4Init | 4.65%（地址待修正）|
| EffectUpdateCallback4 | 0%（地址待修正）|
...
- **新增 2 模块 objdiff 跟踪**：Supervisor (7 函数) + EffectManager (13 函数) = 20 函数
- **7 个函数 ≥90%**：OnDraw 100%, TickTimer 97.95%, Reset 99.92%, EffectCallbackStill 99.93%, EffectCallbackRandomSplashBigInit 99.23%, SpawnParticlesWithVelocity 95.28%, EffectManager::OnUpdate 90.46%
