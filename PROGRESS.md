...
### CMyFont（第 18 模块，5 函数 objdiff，平均 **65.2%**，2×95%+）
orig 3.6KB / 5 函数。th07 文本渲染（GDI-based，非 th06 D3DXFont）。
| 函数 | match% |
|---|---|
| Reset | **99.81%** |
| Clean | **95.00%**（缓存 this + 重读字段 + early return）|
| InitWrapper | 64.94% |
| Init | 63.98% |
...
- **新增 3 模块 objdiff 跟踪**：Supervisor (8 函数) + EffectManager (14 函数) + CMyFont (5 函数) = 27 函数
- **10 个函数 ≥90%**：Supervisor OnDraw 100%/TickTimer 97.95%; EffectManager Reset 99.92%/Still 99.93%/SplashBigInit 99.23%/UpdateCB4Init 99.90%/UpdateCB4 99.19%/SpawnPartWithVel 95.28%/OnUpdate 90.46%; CMyFont Reset 99.81%/Clean 95.00%
